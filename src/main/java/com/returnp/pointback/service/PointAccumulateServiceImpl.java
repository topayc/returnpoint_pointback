...
/*		if (paymentTransaction.getNodeType().equals("1") && paymentTransaction.getAffiliateNo() != null  &&  
	...
	public ReturnpBaseResponse accumulateByAdmin(int paymentTransactionNo, int accType) {
		...
		return res;
			...
			ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_ERROR, "2000", this.messageUtils.getMessage("pointback.message.error"));
			return res;
		}
	}
	...
	public ReturnpBaseResponse processByEncQr(
		...
		try {
			...
			if (members.size() !=1 || members == null) {
				ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK,  "601", this.messageUtils.getMessage("pointback.message.not_member"));
				return res;
			}
			...
			if (!member.getMemberPhone().equals(phoneNumber) && 
				...
				ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK,  "602", this.messageUtils.getMessage("pointback.message.invalid_phonenumber"));
				throw new ReturnpException(res);
			}
			...
			if (pas.equals("0")) {
				if (paymentTransactions.size()> 0) {
					ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "603", this.messageUtils.getMessage("pointback.message.already_register_payment_acc"));
					throw new ReturnpException(res);
				}
			}
			/*사용자의 요청이 결제 취소에 의한 적립 취소 요청일 경우 */
				...
				if (paymentTransactions.size() == 1)  {
					if (!paymentTransactions.get(0).getPaymentApprovalStatus().equals(AppConstants.PaymentApprovalStatus.PAYMENT_APPROVAL_OK)) {
						ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "604", this.messageUtils.getMessage("pointback.message.not_cancelable_paymment_a"));
						throw new ReturnpException(res);
					}
				}else {
					ResponseUtil.setResponse(res,ResponseUtil.RESPONSE_OK,  "605", this.messageUtils.getMessage("pointback.message.not_payment_invalid_req"));
					throw new ReturnpException(res);
				}
			}
			...
			if (affiliates.size() != 1 || affiliates  == null) {
				ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "606", this.messageUtils.getMessage("pointback.message.not_argu_affiliate", new Object[] {afId}));
				throw new ReturnpException(res);
			} 
			
			/* 가맹점 시리얼 고유 번호 비교  */
			if (!affiliates.get(0).getAffiliateSerial().equals(afId)) {
				ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "607", this.messageUtils.getMessage("pointback.message.wrong_serial_number"));
				throw new ReturnpException(res);
			}
			...
			if (pas.equals("0")) {
				/* 결제 상태가 결제 승인이 아니라면 중지 */
				if (!paymentTransaction.getPaymentApprovalStatus().trim().equals(AppConstants.PaymentApprovalStatus.PAYMENT_APPROVAL_OK)) {
					ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "608", this.messageUtils.getMessage("pointback.message.invalid_qr"));
					throw new ReturnpException(res);
				}
				...
				if (paymentTransaction.getPointBackStatus().trim().equals(AppConstants.GreenPointAccStatus.POINTBACK_COMPLETE)  || 
						paymentTransaction.getPointBackStatus().trim().equals(AppConstants.GreenPointAccStatus.POINTBACK_STOP) ) {
					ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "609", this.messageUtils.getMessage("pointback.message.completed_request_acc"));
					throw new ReturnpException(res);
				}
				...
				if (!paymentTransaction.getPaymentApprovalStatus().trim().equals(AppConstants.PaymentApprovalStatus.PAYMENT_APPROVAL_CANCEL)) {
					ResponseUtil.setResponse(res,ResponseUtil.RESPONSE_OK,  "610", this.messageUtils.getMessage("pointback.message.invalid_qr"));
					throw new ReturnpException(res);
				}
				...
				if (paymentTransaction.getPointBackStatus().trim().equals(AppConstants.GreenPointAccStatus.POINTBACK_CANCEL_COMPLETE)  || 
						paymentTransaction.getPointBackStatus().trim().equals(AppConstants.GreenPointAccStatus.POINTBACK_STOP)) {
					ResponseUtil.setResponse(res, ResponseUtil.RESPONSE_OK, "611", this.messageUtils.getMessage("pointback.message.completed_request_acc_cancel"));
					throw new ReturnpException(res);
				}
				return this.accumulatePoint(paymentTransaction, -1);
			}
